...
signed main() {
  ...
  while (t--) {
    ...
    int len = str.length();
    int n = str.length();
    vector<vi> arr(2, vi(len, 0));

    arr[0][0] = (str[0] == '0');
    arr[1][0] = (str[0] == '1');

    for (int i = 1; i < len; i++) {
      arr[0][i] += arr[0][i - 1] + (str[i] == '0');
      arr[1][i] += arr[1][i - 1] + (str[i] == '1');
    }

    auto get = [&](int l, int r, int x) {
      if (l == 0) return arr[x][r];
      return arr[x][r] - arr[x][l - 1];
    };
    ...
    for (int i = 0; i < n; i++) {
      ...
      while (lb < rb) {
        int mb = (lb + rb) / 2;
        if (get(mb, i, 0) > arr[1][n - 1] - get(mb, i, 1))
          lb = mb + 1;
        else
          rb = mb;
      }
      ans = min(ans, max(get(lb, i, 0), arr[1][n - 1] - get(lb, i, 1)));
    }
    cout << ans << endl;
  }
  return 0;
