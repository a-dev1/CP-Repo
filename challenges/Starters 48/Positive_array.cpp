...
signed main() {
  ...
  while (t--) {
    ...
    int ans = 1, totalAssigned = 0;
    int temp = 1;
    for (auto i : mp) {
      int nob = (ans * i.fi) - totalAssigned, nor = i.se;
      if (nor > nob) ans += ceil((nor - nob) / (float)i.fi);

      // if((nor - nob)%i.fi != 0) ans++;

      totalAssigned += i.se;
      if (i.se > (temp * i.fi - i.se))
        temp += (i.se - (ans * i.fi - i.se)) / i.fi;
      if ((i.se - (temp * i.fi - i.se)) % i.fi != 0) temp++;
    }
