...
signed main() {
  ...
  while (q--) {
    ...
    if (type == 1) {
      ...
      row[r]++;
      col[c]++;
      if (rowM.find(r) != rowM.end()) rowM.erase(r);
      if (colM.find(c) != colM.end()) colM.erase(c);
    } else if (type == 2) {
      int r, c;
      cin >> r >> c;
      if (row[r] > 0) row[r]--;
      ...
      if (row[r] == 0) rowM.insert(r);
      if (col[c] == 0) colM.insert(c);
    } else {
      int x1, y1, x2, y2;
      cin >> x1 >> y1 >> x2 >> y2;
      ...
      auto itr = rowM.lower_bound(x1);
      auto itr2 = colM.lower_bound(y1);
      if ((*itr <= x2 && *itr >= x1) && (*itr2 <= y2 && *itr2 >= y1)) {
        cout << "No\n";
        continue;
      } else
        cout << "Yes\n";
