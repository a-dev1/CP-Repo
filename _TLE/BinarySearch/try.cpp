...
signed main() {
  ...
  while (t--) {
    ...
    for (int i = 0; i < n; i++) {
      arr.push_back({inter[i] - dis, inter[i] + dis});
      if (i != 0) {
        if (max(arr[i].fi, arr[i - 1].fi) > min(arr[i].se, arr[i - 1].se)) {
          ans += arr[i].fi - arr[i - 1].se - 1;
        }
      }
    }
    ...
    if (arr[arr.size() - 1].se < 100) {
      ans += (100 - arr[arr.size() - 1].se);
    }
