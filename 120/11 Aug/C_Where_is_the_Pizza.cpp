  ...
  while (t--) {
    ...
    for (int i = 1; i <= n; i++) {
      if (!visited[i]) {
        ...
        bool infected = false;
        // simple Dfs to check cycle & discarding the cycle having elements that
        // are already present in C.
        while (!infected && node != 0 && s.find(node) == s.end()) {
          if (notGood.find(node) == notGood.end())
            good += 1;
          else
            infected = true;
          visited[node] = 1;
